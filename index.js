...
var loadEvents = function(start, end, classroom, callback) {
	...
	var CalendarDataUrl = 'http://cims.nyu.edu/webapps/classrooms/WWH/' + classroom + '/calendar?start=' + start +
			'&end=' + end + '&format=json';
...
var appendClassRoom = function(classroom, data) {
...
var getClassrooms = function(start, end, limitation) {
	...
	for (var i = 0; i < Object.keys(ClassRoomMap).length; i++) {
		if (ClassRoomMap[Object.keys(ClassRoomMap)[i]] <= limitation) {
			loadEvents(start, end, Object.keys(ClassRoomMap)[i], appendClassRoom);
		}
	}
};
